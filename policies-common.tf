...
# API Gateway Policies -- see readme for explination of the use of count
resource "oci_identity_policy" "APIGatewayAccessPolicy" {
  ...
  compartment_id = var.policy_compartment_ocid
  statements = ["Allow group ${var.policy_for_group} to manage api-gateway-family in compartment '${var.policy_compartment_ocid}'",
  "Allow group ${var.policy_for_group} to manage api-gateway-family in compartment '${var.policy_compartment_ocid}'"]
  defined_tags = var.defined_tags
}
...
resource "oci_identity_policy" "OKEAccessPolicy" {
  ...
  # see module readme for more information on what is happening here

  depends_on     = [oci_identity_dynamic_group.FunctionsServiceDynamicGroup]
  name           = "APIGatewayAccessPolicy-${var.random_id}"
  description    = "APIGatewayAccessPolicy-${var.random_id}"
  compartment_id = var.policy_compartment_ocid
  statements = ["Allow group ${var.policy_for_group} to manage instance-family in '${var.policy_compartment_ocid}'",
  "Allow group ${var.policy_for_group} to use subnets in  '${var.policy_compartment_ocid}'"]
  defined_tags = var.defined_tags
}
