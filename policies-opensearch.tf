...
resource "oci_identity_policy" "open_search_network_policy" {
  ...
  description    = "OpenSearchNetworkPolicy-${var.random_id} - allow OpenSearch to interact with networking"
...
resource "oci_identity_policy" "open_search_clusters_policy" {
  ...
  description    = "OpenSearchClustersPolicy-${var.random_id} - group interact with opensearch family"
  compartment_id = var.policy_compartment_ocid
...
resource "oci_identity_policy" "open_search_clusters_user_policy" {
  ...
  description    = "OpenSearchClustersUserPolicy-${var.random_id} - user manage OpenSearch policy family"
  compartment_id = var.policy_compartment_ocid
